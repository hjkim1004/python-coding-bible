...
강은 **«이 강에서 배우는 것»** 으로 연다. 그 차례는 손으로 적지 않고 **절 제목에서 그대로
뽑으므로** 본문을 고치면 함께 따라온다. 차례의 항목을 누르면 그 절로 가고, 주소도
`#/p1-list/s5` 처럼 절까지 남아 **«거기 5절»** 을 링크로 줄 수 있다.

문법 강에는 **메서드 표**가 있다 — 하는 일과 복잡도만이 아니라 **«돌려주는 값»** 까지
적었다. `arr.sort()` 가 `None` 을 돌려준다는 것을 모르면 리스트를 통째로 잃는다.
...
강 번호가 화면과 목차에서 어긋나지 않는지, 본문이 «3-6에서 봤듯이» 하고 **없는 강을
가리키지 않는지**도 함께 본다. 강을 사이에 끼워 넣으면 번호가 밀리고, 그때 그런 문장이
조용히 거짓이 된다.
